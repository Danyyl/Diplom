...
import {get_user_page, login, setToken, takeUser, takeUsers} from '../redux/reducers/auth/authActions'
...
export default function ServiceUserB() {
  const classes = useStyles();
  const token = useSelector(({authReducer}) => authReducer.token)
  const history = useHistory();
  const dispatch = useDispatch();
  const temp_user = useSelector(({authReducer}) => authReducer.temp_user)
  ...
  try {
    ...
    const request = axios.put(`http://0.0.0.0:8080/api/service/` + service.id + "/", requestBody, {
        ...
        request.then((response) => {
            update_done();
            dispatch(get_user_page(token, temp_user, history));
        })
        .catch(error => {
            console.log(error);
        });
  ...
  function add_input() {
    ...
    const request = axios.post(`http://0.0.0.0:8080/api/input_data/`, requestBody, {
                        headers: { Authorization: `Bearer ${token}` }
                    });

        request.then((response) => {
            service.inputs.push(response.data)
            get_inputs()
        })
        .catch(error => {
            console.log(error);
          ...
          <div className={classes.top}>
              ...
              <div className={classes.container}>
                  <Typography className={classes.description}>Inputs</Typography>
                    {inputs}
                  <button className={classes.edit_button} onClick={() => {add_input()}}>Add</button>
                </div>
                <div className={classes.container}>
